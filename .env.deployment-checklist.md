# 🔑 CHECKLIST DE VARIABLES DE ENTORNO - V3.1.2

**Para deployment en Supabase Edge Functions**

---

## 📋 VARIABLES REQUERIDAS

### **1. STRIPE (CRÍTICO)**

```bash
# Stripe Secret Key (empieza con sk_test_ o sk_live_)
STRIPE_SECRET_KEY=sk_test_xxxxx

# Stripe Webhook Secret (empieza con whsec_)
# NOTA: Este se genera DESPUÉS de crear el webhook endpoint
STRIPE_WEBHOOK_SECRET_V312=whsec_xxxxx
```

**¿Dónde encontrarlas?**
- Secret Key: https://dashboard.stripe.com/test/apikeys
- Webhook Secret: Se genera después (Paso 1.4 del Runbook)

---

### **2. SUPABASE (CRÍTICO)**

```bash
# URL de tu proyecto Supabase
SUPABASE_URL=https://xxxxx.supabase.co

# Anon Key (pública, empieza con eyJ)
VITE_SUPABASE_ANON_KEY=eyJxxxxx

# Service Role Key (privada, empieza con eyJ)
SUPABASE_SERVICE_ROLE_KEY=eyJxxxxx
```

**¿Dónde encontrarlas?**
- Dashboard > Settings > API
- URL: Project URL
- Anon Key: anon/public key
- Service Role: service_role key (¡NUNCA expongas esta!)

---

### **3. PRICING CONFIG (CRÍTICO)**

```bash
# Fees de Stripe (worst case scenario)
STRIPE_WORST_CASE_FEE_PERCENT=3.5
STRIPE_WORST_CASE_FEE_FIXED_EUR=0.25
```

**Valores por defecto:** Ya están arriba ✅

---

### **4. NOTIFICACIONES (OPCIONAL - Para después)**

```bash
# Twilio (SMS/WhatsApp)
TWILIO_ACCOUNT_SID=ACxxxxx
TWILIO_AUTH_TOKEN=xxxxx
TWILIO_WHATSAPP_FROM=whatsapp:+14155238886

# SendGrid (Email)
SENDGRID_API_KEY=SG.xxxxx
SENDGRID_FROM_EMAIL=noreply@parisluxejourney.com
```

**Nota:** Puedes configurar esto después. No es crítico para testing inicial.

---

## ✅ CHECKLIST DE VARIABLES

### **MÍNIMO PARA STAGING:**
- [ ] `STRIPE_SECRET_KEY` (test mode)
- [ ] `SUPABASE_URL`
- [ ] `VITE_SUPABASE_ANON_KEY`
- [ ] `SUPABASE_SERVICE_ROLE_KEY`
- [ ] `STRIPE_WORST_CASE_FEE_PERCENT` (3.5)
- [ ] `STRIPE_WORST_CASE_FEE_FIXED_EUR` (0.25)

### **PARA WEBHOOKS (Después):**
- [ ] `STRIPE_WEBHOOK_SECRET_V312` (se genera al crear webhook)

### **PARA NOTIFICACIONES (Opcional):**
- [ ] `TWILIO_ACCOUNT_SID`
- [ ] `TWILIO_AUTH_TOKEN`
- [ ] `TWILIO_WHATSAPP_FROM`
- [ ] `SENDGRID_API_KEY`
- [ ] `SENDGRID_FROM_EMAIL`

---

## 📝 FORMATO PARA COPIAR

Cuando tengas el .env del otro PC, copia estas líneas:

```bash
# Del archivo .env del otro PC, busca y copia:

# 1. Stripe
STRIPE_SECRET_KEY=
VITE_STRIPE_PUBLISHABLE_KEY=

# 2. Supabase
SUPABASE_URL=
VITE_SUPABASE_ANON_KEY=
SUPABASE_SERVICE_ROLE_KEY=

# 3. Twilio (si existe)
TWILIO_ACCOUNT_SID=
TWILIO_AUTH_TOKEN=
TWILIO_WHATSAPP_FROM=

# 4. SendGrid (si existe)
SENDGRID_API_KEY=
SENDGRID_FROM_EMAIL=
```

---

## 🎯 CÓMO CONFIGURARLAS EN SUPABASE

### **Opción 1: Dashboard (Manual)**

1. Ir a: https://supabase.com/dashboard/project/[tu-proyecto]
2. Click en **Settings** (menú izquierdo)
3. Click en **Edge Functions**
4. Scroll hasta **"Secrets"**
5. Click **"Add new secret"**
6. Nombre: `STRIPE_SECRET_KEY`
7. Valor: `sk_test_xxxxx`
8. Click **"Save"**
9. Repetir para cada variable

### **Opción 2: CLI (Más rápido)**

```bash
# Configurar todas de una vez
supabase secrets set STRIPE_SECRET_KEY=sk_test_xxxxx
supabase secrets set SUPABASE_URL=https://xxxxx.supabase.co
supabase secrets set VITE_SUPABASE_ANON_KEY=eyJxxxxx
supabase secrets set SUPABASE_SERVICE_ROLE_KEY=eyJxxxxx
supabase secrets set STRIPE_WORST_CASE_FEE_PERCENT=3.5
supabase secrets set STRIPE_WORST_CASE_FEE_FIXED_EUR=0.25

# Verificar que se guardaron
supabase secrets list
```

---

## ⚠️ IMPORTANTE - SEGURIDAD

### **NUNCA expongas:**
- ❌ `SUPABASE_SERVICE_ROLE_KEY` en frontend
- ❌ `STRIPE_SECRET_KEY` en frontend
- ❌ `TWILIO_AUTH_TOKEN` en frontend

### **SÍ puedes exponer:**
- ✅ `VITE_SUPABASE_ANON_KEY` (es pública)
- ✅ `VITE_STRIPE_PUBLISHABLE_KEY` (es pública)
- ✅ `SUPABASE_URL` (es pública)

---

## 🧪 TEST MODE vs LIVE MODE

### **Para STAGING/TESTING:**
```bash
STRIPE_SECRET_KEY=sk_test_xxxxx  # ← Empieza con sk_test_
```

### **Para PRODUCCIÓN:**
```bash
STRIPE_SECRET_KEY=sk_live_xxxxx  # ← Empieza con sk_live_
```

**IMPORTANTE:** Usa **TEST MODE** hasta que hayas completado los 7 tests E2E.

---

## 📞 CUANDO TENGAS LAS CREDENCIALES

Avísame y te ayudo a:
1. ✅ Configurarlas en Supabase
2. ✅ Verificar que están correctas
3. ✅ Deploy de Edge Functions
4. ✅ Configurar webhook de Stripe

---

**Fecha:** _____________  
**Credenciales copiadas:** ☐ SÍ  ☐ NO  
**Configuradas en Supabase:** ☐ SÍ  ☐ NO

