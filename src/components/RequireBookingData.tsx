...
export const RequireBookingData = ({ children }: { children: React.ReactNode }) => {
  const { t } = useLanguage();
  const { toast } = useToast();
  const location = useLocation();
  const { bookingData, estimatedPrice, priceTimestamp, updateBookingData } = useBooking();
  ...
  useEffect(() => {
    const processNavigationData = async () => {
      if (navigationBookingData && !bookingData) {
        console.log('[RequireBookingData] Encontrados datos de reserva en el estado de navegación:', navigationBookingData);
        console.log('[RequireBookingData] Precio estimado en navegación:', navigationPrice);
        ...
        const updatedBookingData = {
          ...
          luggageSurcharge: navigationBookingData.luggageSurcharge || (() => {
            const largeLuggage = Number(navigationBookingData.largeLuggageCount) || 0;
        ...
        console.log('[RequireBookingData] Datos de reserva actualizados con precio:', updatedBookingData);
        
        try {
          // Actualizar el contexto con los datos de la navegación
          await updateBookingData(updatedBookingData);
          console.log('RequireBookingData - Contexto actualizado correctamente');
        } catch (error) {
          console.error('RequireBookingData - Error al actualizar contexto:', error);
        }
      }
    };
  ...
  if (!bookingData && !navigationBookingData) {
    console.log('RequireBookingData - No se encontraron datos de reserva, redirigiendo a /booking');
    toast({
  ...
  if (priceTimestamp && (now - priceTimestamp > PRICE_STALE_THRESHOLD)) {
    console.log('RequireBookingData - Precio obsoleto, redirigiendo a /booking');
    toast({
  ...
  console.log('RequireBookingData - Datos de reserva válidos, mostrando children');
  return <>{children}</>;
};
