...
export const useBookingForm = () => {
  ...
  const { price, distance, calculatePrice, handleDistanceCalculated } =
    useBookingPrice();
  ...
  const handleChange = (
    ...
    if ((typeof e === "string" || typeof e === "number") && fieldName) {
      const value = ["largeLuggageCount", "smallLuggageCount"].includes(
        fieldName,
      )
        ? Number(e) || 0
        : e;
    ...
    const isLocationChange =
      ...
      fieldName === "dropoff" ||
      (typeof e === "object" &&
        (e.target.name === "pickup" || e.target.name === "dropoff"));
  ...
  // Capacidades: Berlina (1-3 pax) máx 3 grandes, Van (4-7 pax) máx 7 grandes
  const calculateTotalPrice = useCallback(
    (
      ...
      // Calculamos el recargo usando la función centralizada
      const surcharge = calculateLuggageFee(passengers, luggage);
  ...
  // Función simplificada para calcular solo el recargo (usada en otros lugares)
  const calculateLuggageSurcharge = useCallback(
    (largeLuggage: number, passengerCount: number) => {
      return calculateLuggageFee(passengerCount, largeLuggage);
    },
    [],
  );
