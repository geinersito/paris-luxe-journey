...
**main HEAD:** `cd27d41` — 2026-08-02

**DEPLOY NOTE:** Corrección: producción (eliteparistransfer.com) se hospeda en **Hostinger**, no Netlify (nota anterior incorrecta). Desplegado y verificado el 2026-08-02 vía SFTP manual siguiendo `docs/runbooks/HOSTINGER_ROOT_ERP_DEPLOY.md` — producción coincide con `main` en `cd27d41`. Hallazgo previo (`OPS-PROD-DEPLOY-PREFLIGHT-01`): producción llevaba 16 commits de retraso (desde `2c08e95`, 2026-06-04) sin que ningún deploy anterior lo hubiera sincronizado.

## Ahora en curso

- **OPS-PROD-DEPLOY-SSH-RUNBOOK-01** (P0/R0) — **DOING** — Documenta el método persistente de despliegue SSH en `docs/runbooks/HOSTINGER_ROOT_ERP_DEPLOY.md`: alias local esperado `eliteparis-hostinger`, mapeo de destinos (booking/erp/beta-erp/tasks), transferencia obligatoria en dos fases (`index.html` siempre al final, `.htaccess` excluido, sin `--delete`/`rm`/limpieza remota), backup previo, smoke post-deploy y procedimiento de rollback (antes inexistente). Resuelve el hallazgo de `OPS-PROD-DEPLOY-PREFLIGHT-01`. Solo documentación — sin IP, usuario, puerto ni fingerprints (viven únicamente en el `~/.ssh/config` local de cada operador). PR: [#250](https://github.com/geinersito/paris-luxe-journey/pull/250).

- **EVENTS-TEMPORAL-INTEGRITY-01** (P0/R1) — **DOING** — SSOT único de clasificación temporal de eventos (`src/lib/events/classify.ts`, consciente de Europe/Paris): un evento finalizado desaparece inmediatamente de Home y de las listas actuales, se excluye de `EventScheduled` en JSON-LD, y solo aparece en "Recently Ended" durante la gracia existente de 7 días. "This Week"/"This Month" ahora se derivan del calendario real de Europe/Paris (solapamiento de rango semana/mes), no de los arrays congelados `thisWeek`/`thisMonth` de `events-feed.json`. La misma colección elegible deduplicada alimenta UI y JSON-LD. Resuelto de la auditoría `EVENTS-REFRESH-2026-01`. 24 tests nuevos con reloj fijado. No toca `events-feed.json`, imágenes, copy, traducciones, enlaces, CTA ni diseño de tarjetas — eso llega con `EVENTS-DATA-REFRESH-01`/`EVENTS-UX-REDESIGN-01`/`EVENTS-CONTENT-IMAGES-01`. PR: [#245](https://github.com/geinersito/paris-luxe-journey/pull/245).
- **CONTENT-FAQ-PRIVATEDRIVER-24H-01A** (P0/R0) — **DOING** — Corrige `faq.questions.privateDriver.answer` en EN/ES/PT: retira la promesa no respaldada de "24/7 assistance", conserva solo prestaciones verificables (chófer, vehículo, agua, wifi) y reformula los servicios adicionales como bajo solicitud y sujetos a confirmación. Resuelto de la auditoría `CONTENT-FAQ-24H-CLAIMS-01`: era la única respuesta del FAQ que bloqueaba directamente `PR-SEO-FAQ-SCHEMA-01`. FR sin tocar — nunca tuvo el claim 24/7 (su divergencia editorial se trata aparte). Sin cambios en CTA, footer, badges, Terms, aeropuertos, email templates ni código muerto. PR: [#244](https://github.com/geinersito/paris-luxe-journey/pull/244).
- **CONTENT-FAQ-ADVANCE-NOTICE-01B** (P0/R0) — **DOING** — Corrige `faq.questions.advanceBooking.answer` en FR: retira el claim de "24 heures pour garantir la disponibilité", remite a revisión manual de itinerario/vehículo y marca las solicitudes con poca antelación como "sur demande", no garantizadas. Completa la alineación de anticipación EN/FR/ES/PT (sigue a `01A`, PR [#242](https://github.com/geinersito/paris-luxe-journey/pull/242)). `PR-SEO-FAQ-SCHEMA-01` sigue BLOCKED únicamente por la auditoría/corrección de claims 24/7. PR: [#243](https://github.com/geinersito/paris-luxe-journey/pull/243).
...
- **CONTENT-FAQ-PAYMENT-SSOT-01B** (P0/R0) — **DONE ✅** — Corrige `faq.questions.deposit.answer` en FR: retira el claim no respaldado de "acompte du 30% + solde", alineado con `TermsPage.tsx` ("100% online payment required") y `pricing.payment.method = "100%_online"`. Completa la alineación EN/FR/ES/PT con Terms/pricing (sigue a `01A`, PR [#238](https://github.com/geinersito/paris-luxe-journey/pull/238)). `PR-SEO-FAQ-SCHEMA-01` sigue BLOCKED — cancelación, anticipación y 24/7 siguen pendientes. PR: [#239](https://github.com/geinersito/paris-luxe-journey/pull/239).
