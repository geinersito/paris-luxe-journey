...
- **CONTENT-FAQ-FLEET-CONSISTENCY-01** (P2/R0) — **DOING** — Retira BMW y los claims no validados de "menos de 2 años" / SUV de la respuesta `faq.questions.vehicleTypes` en EN/ES/PT (FR ya estaba libre de estos claims, sin tocar). Cambio de copy/i18n únicamente; requisito previo antes de autorizar `PR-SEO-FAQ-SCHEMA-01` para no amplificar vía datos estructurados una afirmación de flota inconsistente. Branch: `fix/content-faq-fleet-consistency-01`.
