...
|---|---|---|---|---|---|---|---|
| OPS-PROD-DEPLOY-SSH-RUNBOOK-01 | P0 | R0 | Ops/Deploy | Document the persistent SSH deploy method (expected local alias, destination mapping, mandatory two-phase transfer, backup/rollback) in the canonical Hostinger runbook | DOING | [#250](https://github.com/geinersito/paris-luxe-journey/pull/250) | Resolved from `OPS-PROD-DEPLOY-PREFLIGHT-01` audit: production was 16 commits behind `main` with no documented, reproducible deploy method. Docs-only — no code, no secrets, no IP/user/port/fingerprints recorded (those live only in each operator's local `~/.ssh/config`). Alias name: `eliteparis-hostinger`. |
| EVENTS-TEMPORAL-INTEGRITY-01 | P0 | R1 | Events/Data | Single SSOT for event temporal classification (`src/lib/events/classify.ts`, Europe/Paris-aware): a finished event now disappears immediately from Home and current lists, is excluded from `EventScheduled` JSON-LD, and only surfaces in "Recently Ended" during the existing 7-day grace. "This Week"/"This Month" are now derived from the real Europe/Paris calendar (week/month range overlap), not from the frozen `thisWeek`/`thisMonth` arrays in `events-feed.json`. Same de-duplicated eligible collection now backs UI and JSON-LD | DOING | [#245](https://github.com/geinersito/paris-luxe-journey/pull/245) | Resolved from `EVENTS-REFRESH-2026-01` audit. 24 new unit tests (boundary-precise, fixed clock `2026-08-01T12:00:00+02:00`). Does not touch `events-feed.json` data, images, copy, translations, links, CTA or card design — those are `EVENTS-DATA-REFRESH-01` / `EVENTS-UX-REDESIGN-01` / `EVENTS-CONTENT-IMAGES-01`, queued after this. |
| CONTENT-FAQ-PRIVATEDRIVER-24H-01A | P0 | R0 | Content/i18n | Fix FAQ `faq.questions.privateDriver.answer` (EN/ES/PT): remove unsupported "24/7 assistance" promise, keep only verifiable existing features (chauffeur, vehicle, water, wifi), reframe additional services as on-request/subject to confirmation | DOING | [#244](https://github.com/geinersito/paris-luxe-journey/pull/244) | Resolved from `CONTENT-FAQ-24H-CLAIMS-01` audit: this was the sole FAQ answer directly blocking `PR-SEO-FAQ-SCHEMA-01`. FR untouched — it never contained the 24/7 claim (its content diverges editorially, tracked separately). No CTA/footer/badges/Terms/airports/email-template/dead-code changes in this slice. |
| CONTENT-FAQ-ADVANCE-NOTICE-01B | P0 | R0 | Content/i18n | Fix FAQ `faq.questions.advanceBooking.answer` (FR): remove "24 heures pour garantir la disponibilité" claim, defer to manual itinerary/vehicle review, mark short-notice as "sur demande" and not guaranteed | DOING | [#243](https://github.com/geinersito/paris-luxe-journey/pull/243) | Completes EN/FR/ES/PT advance-notice alignment (follows `01A`, [#242](https://github.com/geinersito/paris-luxe-journey/pull/242)). `PR-SEO-FAQ-SCHEMA-01` remains BLOCKED only by the 24/7 claims audit/fix. |
| CONTENT-FAQ-ADVANCE-NOTICE-01A | P0 | R0 | Content/i18n | Fix FAQ `faq.questions.advanceBooking.answer` (EN/ES/PT): remove conflicting 24h(FR)/48h(EN/ES/PT) arbitrary windows and unsupported last-minute-availability promise; defer to manual itinerary/vehicle review, mark short-notice as "on request / bajo solicitud / sob consulta" and not guaranteed | DONE | [#242](https://github.com/geinersito/paris-luxe-journey/pull/242) | Resolved from `AUDIT-FAQ-CLAIMS-01`: does not introduce a fictitious minimum-notice window not enforced by any booking guard/availability check. `01B` (FR) follows immediately. `PR-SEO-FAQ-SCHEMA-01` remains BLOCKED by FR and 24/7 claims. |
| CONTENT-FAQ-CANCELLATION-SSOT-01B | P0 | R0 | Content/i18n | Fix FAQ `faq.questions.cancellation.answer` (FR): remove unsupported "24h entièrement remboursé" claim, defer to Conditions + contact, clarify refund is not automatic | DONE | [#241](https://github.com/geinersito/paris-luxe-journey/pull/241) | Completes EN/FR/ES/PT cancellation alignment (follows `01A`, [#240](https://github.com/geinersito/paris-luxe-journey/pull/240)). `PR-SEO-FAQ-SCHEMA-01` remains BLOCKED by advance-notice (24h/48h) and 24/7 claims. |
| CONTENT-FAQ-CANCELLATION-SSOT-01A | P0 | R0 | Content/i18n | Fix FAQ `faq.questions.cancellation.answer` (EN/ES/PT): remove unsupported binary "24h free / 50% fee" claim (conflicts with the 3-tier Terms/Confirmation policy and with the "not defined" refund flow in `BOOKING_MODEL.md`); defer to Terms + contact, clarify refund is not automatic | DONE | [#240](https://github.com/geinersito/paris-luxe-journey/pull/240) | Resolved from `AUDIT-FAQ-CLAIMS-01`: does not copy the Terms 3-tier schedule into FAQ (would imply an automated refund process that does not exist per `BOOKING_MODEL.md` §5 "Cancellation flow: Not defined") nor treat `pricing.ts` `cancelPolicy` as enforced SSOT (it is unconsumed decorative config). `01B` (FR) follows immediately. `PR-SEO-FAQ-SCHEMA-01` remains BLOCKED by FR, advance-notice (24h/48h) and 24/7 claims. |
| CONTENT-FAQ-PAYMENT-SSOT-01B | P0 | R0 | Content/i18n | Fix FAQ `faq.questions.deposit.answer` (FR): remove unsupported "30% acompte / solde later" claim, align with `TermsPage.tsx` ("100% online payment required") and `pricing.payment.method = "100%_online"` | DONE | [#239](https://github.com/geinersito/paris-luxe-journey/pull/239) | Completes EN/FR/ES/PT payment alignment with Terms/pricing (follows `01A`, [#238](https://github.com/geinersito/paris-luxe-journey/pull/238)). `PR-SEO-FAQ-SCHEMA-01` remains BLOCKED — cancellation, advance-notice and 24/7 claims still pending (`CONTENT-FAQ-CANCELLATION-SSOT-01`, `CONTENT-FAQ-ADVANCE-NOTICE-01`, `CONTENT-FAQ-24H-CLAIMS-*`). |
| CONTENT-FAQ-PAYMENT-SSOT-01A | P0 | R0 | Content/i18n | Fix FAQ `faq.questions.deposit.answer` (EN/ES/PT): remove unsupported "30% deposit / balance later" claim, align with `TermsPage.tsx` ("100% online payment required") and `pricing.payment.method = "100%_online"` | DONE | [#238](https://github.com/geinersito/paris-luxe-journey/pull/238) | Resolved from `AUDIT-FAQ-CLAIMS-01`: FAQ deposit claim was in direct RUNTIME-MISMATCH with Terms & Conditions and pricing config — no product/Confirmation/Stripe changes in this slice. `01B` (FR) follows immediately; then `CONTENT-FAQ-CANCELLATION-SSOT-01`, `CONTENT-FAQ-ADVANCE-NOTICE-01`, `CONTENT-FAQ-24H-CLAIMS-*`. `PR-SEO-FAQ-SCHEMA-01` remains BLOCKED until this sequence completes. |
| CONTENT-FAQ-FLEET-CONSISTENCY-01 | P2 | R0 | Content/i18n | Remove BMW and unvalidated "<2 years old"/SUV claims from FAQ `vehicleTypes` answer (EN/ES/PT) | DONE | [#237](https://github.com/geinersito/paris-luxe-journey/pull/237) | FR already free of these claims, left untouched. Copy-only i18n change; enables `PR-SEO-FAQ-SCHEMA-01` without amplifying inconsistent fleet claims via structured data. |
| OPS-HUSKY-HOOKS-01 | P0 | R0 | Ops/Tooling | Fix Git hook file modes and remove unsafe auto-install hook | DONE | [#236](https://github.com/geinersito/paris-luxe-journey/pull/236) | `pre-commit` and `pre-push` restored `100644` → `100755` (were never executable since initial commit, silently skipping lint-staged/type-check/build gates locally); `post-merge` (bare `npm install` on every merge/pull, unreviewed third-party install scripts) removed — recoverable from Git history. No changes to product code or dependencies. |
...
| EVENTS-QUICK-FILTERS-SIDEBAR-01 | P2 | R0 | UX/Navigation | Add Excursions-style quick filters sidebar in Events with section jump navigation | DONE | [#139](https://github.com/geinersito/paris-luxe-journey/pull/139) / `61ccf5eca98beef1318c2d21dd2fe4cd1ce06225` | Added sidebar quick navigation in `src/pages/Events.tsx` with smooth scroll anchors (`events-week`, `events-month`) and active-state highlighting via `IntersectionObserver`; layout/navigation-only |
| EVENTS-SPACE-DENSITY-01 | P2 | R0 | UX/Layout | Tighten vertical rhythm on Events listings/trust/CTA blocks to reduce excessive whitespace | DONE | [#141](https://github.com/geinersito/paris-luxe-journey/pull/141) / `8478715bd40245ac3a013f6a3f862ff9a5e0b005` | Spacing pass in `src/pages/Events.tsx` (section paddings, inter-block gaps, header margins) with no logic/data changes and preserved sidebar behavior |
| BLOG-HERO-SIDEBAR-CONSISTENCY-01 | P2 | R0 | UX/Blog | Align Blog visual structure with Events/Excursions: clamp hero media and use desktop 8/4 sidebar layout | DONE | [#142](https://github.com/geinersito/paris-luxe-journey/pull/142) / `050b179edd3fbba0b2fc4982c8cdcdf2efc8d2bb` | Updated `BlogHeader` media sizing (aspect + max-height) and `BlogCategory` layout to include sticky sidebar; layout-only, responsive preserved |
...
| PR-SEO-HOME-01 | P0 | R0 | SEO | Fix Home og:image path (.jpg→.png) + canonical trailing slash. Helmet already present with canonical/OG/Twitter/JSON-LD. H1 confirmed in HeroSection. | DONE | `f3bfbef` | og-image `.jpg`→`.png` (2 refs). Canonical `siteOrigin`→`${siteOrigin}/`. 12/12 smoke ✅. |
| PR-SEO-COMMERCIAL-PAGES-01 | P1 | R0 | SEO | Add Helmet (title/desc/canonical/OG/Twitter/JSON-LD) to Excursions + FAQ + Booking. Fix og:image `.jpg`→`.png` globally (5 remaining files). | DONE | `60e6d0d` | 30/30 smoke ✅. |
| PR-SEO-FLEET-CANONICAL-01 | P1 | R0 | SEO | Fix Fleet canonical pointing to `/` instead of `/fleet` | IN REVIEW | PR #234 | FleetPage.tsx replaces HashRedirect. canonical = /fleet ✅. |
...
| OPS-STRIPE-LEGACY-DEPRECATE-01 | — | P2 | R0→R1 | DONE | Deprecate legacy stripe-webhooks handler | [#78](https://github.com/geinersito/paris-luxe-journey/pull/78) / `8fd3365` (docs), [#79](https://github.com/geinersito/paris-luxe-journey/pull/79) / `d7c8606` (telemetry), `86793ea` (PR3 delete) | PR1 ✅ PR2 ✅ PR3 ✅ — both legacy handlers deleted after 108-day no-hit window. Canonical: `supabase/functions/stripe-webhooks/index.ts` only. |
| PR-EV-INGEST-01A | — | P1 | R1 | MERGED-CODE/DEPLOY-HOLD | DB schema: content_event ingest pipeline (sources / ingest_runs / raw / normalized / published) | PR #176 (via branch-rescue) `bd48d89` | Code + migration on main as of 2026-04-28 (included via branch-rescue PR #176). **DB NOT applied. Function NOT deployed. Runtime NOT active.** Deploy gate required: see PLJ-EV-INGEST-01A-DEPLOY-GATE. |
| PLJ-SSOT-001 | P0 | R0 | Governance | Add plan items PLJ-ASSETS/TRANS/OPS/COMP/UI/DB-001/DB-002 + update STATUS.md | DONE | — | Trazabilidad §1.1 — sin este PR ningún PR del batch feat/ev-ingest-01b es abrible |
| PLJ-ASSETS-001 | P1 | R0 | Assets | Favicon + PWA icon refresh: apple-icon-180.png, favicon.ico, manifest-icon-192/512.maskable.png, manifest.json, index.html branding | DONE | `35270a7` | CI + browser favicon load + manifest validates |
| PLJ-TRANS-001 | P1 | R0 | i18n | i18n types + ParisFashion guide copy: src/types/i18n.ts (+53) + en/es/fr/pt (+124 each) | DONE | `c05611f` | tsc --noEmit passes; all keys valid; blocks PLJ-UI-001 + PLJ-COMP-001 |
| PLJ-OPS-001 | P1 | R0 | Ops | Staging build script fix (node scripts/build-env.cjs) + Supabase CLI bump v2.75→v2.90 in .temp/cli-latest | DONE | `b0c5e62` | Staging build command runs OK; no runtime changes |
| PLJ-OPS-002 | P2 | R0 | Ops | gitignore hygiene: supabase/.temp/linked-project.json + elite-root-deploy-*.zip + docs/smoke/ | DONE | pending commit | .gitignore only; no runtime changes |
...
| SEO-INTERNALLINK-RULES-01 | — | P2 | R0 | DONE | Audit + enforce internal linking rules | `3dcc22e` + `ce2ba56` (hotfix cross-link paths) | 5 EN blog articles: `/airports/cdg` + cross-links to related articles added. `/transport/` segment hotfixed in 2 cross-links. |
...
| CONTENT-HOURLY-ITINERARIES-01 | — | P2 | R0 | DONE | Full `/hourly` page rewrite — use cases, pricing, FAQ + JSON-LD | `601a279` | 5 use cases, pricing guardrails (from €75/h, min 3h), FAQ accordion, FAQPage + Service JSON-LD, internal links to /excursions/versailles + /events + /airports/cdg, EN/ES/FR/PT i18n (45+ keys). |
| CONTENT-HOURLY-CTA-POLISH-01 | — | P1 | R0 | DONE | FloatingWA hourly-specific prefill + hero button i18n (4 langs) | `4600b90` | FloatingWA uses `buildHourlyWhatsAppUrl` on `/hourly` route; hero Versailles button i18nized via `hourly.heroCtaVersailles` key (EN/ES/FR/PT). |
| FUNNEL-CTA-REWIRE-01 | — | P0 | R1 | DONE | Replace all `/booking` navigate CTAs with contextual WA prefill | `7f0a007` | 17 files: airport×6, excursion×8, blog/sections×6, guides×2, NotFound, booking onSubmit. `genericBody` neutral (4 langs). 0 user-facing navigate("/booking") remaining. |
