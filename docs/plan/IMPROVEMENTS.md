...
| PR-EV-INGEST-01A | — | P1 | R1 | DOING | DB schema: content_event ingest pipeline (sources / ingest_runs / raw / normalized / published) | draft PR pending | 5 tables, RLS closed (service_role only), append-only raw, promote guardrail, timezone SSOT Europe/Paris. Local full replay gate BLOCKED by SUPABASE-SHARED-REPLAY-DEBT-01. Remote dry-run passed. Not authorized for db push --yes yet. Exception: architectural constraint (schema raw→normalized→published must be atomic). |
...
| PLJ-COMP-001 | P1 | R0 | UI | TestimonialSection i18n-driven refactor (stats: 4.9→4.8 / 100%→98%), TestimonialCard simplify, ContactSection address → Vanves 92170, Privacy+Terms address update | TODO | — | Visual: testimonials + contact render; no console errors. Requires PLJ-TRANS-001 |
| PLJ-UI-001 | P1 | R0 | UI | ParisFashion editorial guide page (src/pages/guides/ParisFashion.tsx) + lazy route in App.tsx | TODO | — | Page loads in all 4 locales; route lazy-loads; no broken keys. Requires PLJ-TRANS-001 |
