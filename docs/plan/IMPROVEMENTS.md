...
| UX-BRANDING-APPROACHABLE-01 | P1 | R0 | Branding/UX | Shift visual perception toward "professional trustworthy" with subtle premium touches (reduce price intimidation) | DOING | [#114](https://github.com/geinersito/paris-luxe-journey/pull/114) / `906a898` | KPI (n>=10 internal review): <=30% "intimidating/expensive", >=70% "clear + trustworthy + accessible". PR1 complete (tokens/vars, merged): baseline palette shifted to professional trustworthy without page/layout edits. PR2 now in progress: centralized component baseline (Button/Card/Badge/Input) with strict no-pages guardrail. DoD: token map + AA contrast checks + before/after desktop/mobile on Home, Airports, Excursions, Events, Fleet. References: `docs/PLAN_CORRECCION_COLORES.md`, `docs/RESUMEN_CORRECCION_COLORES.md`, `docs/PLAN_VIVO_MEJORAS_UI.md`. |
