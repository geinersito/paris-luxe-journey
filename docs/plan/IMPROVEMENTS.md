...
| BOOKING-REQUEST-INTAKE-01 | P0 | R2-lite | Funnel/Booking | Bridge intake table + edge function + /booking/pending contact page + Telegram notification to Boris | DOING | pending PR | `public_booking_requests` table (no dispatch_* coupling); `submit-booking-request` edge fn; BookingForm delegates to onSubmit; /booking/pending 2-step (contact form → confirmation); Telegram sendMessage read-only. Gates: tsc ✅ build ✅. Deploy: supabase db push + functions deploy + TELEGRAM_BOT_TOKEN + TELEGRAM_ALLOWED_CHAT_ID secrets. |
