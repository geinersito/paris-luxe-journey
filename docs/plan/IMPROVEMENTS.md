...
|---|---|---|---|---|---|---|---|
| OPS-PROD-DEPLOY-SSH-RUNBOOK-01 | P0 | R0 | Ops/Deploy | Document the persistent SSH deploy method (expected local alias, destination mapping, mandatory two-phase transfer, backup/rollback) in the canonical Hostinger runbook | DOING | (pending) | Resolved from `OPS-PROD-DEPLOY-PREFLIGHT-01` audit: production was 16 commits behind `main` with no documented, reproducible deploy method. Docs-only — no code, no secrets, no IP/user/port/fingerprints recorded (those live only in each operator's local `~/.ssh/config`). Alias name: `eliteparis-hostinger`. |
